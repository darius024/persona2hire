...
Converts CV dicts into 31 numerical features for ML processing:
- Education (5): level, field_match, university_prestige, years, has_masters
- Work (6): total_years, positions, seniority, field_match, current, tenure
- Skills (5): required_match, extra_match, driving_license, computer_count, total_words
- Languages (4): count, has_english, max_level, mother_tongue_tier
- Soft Skills (3): categories_matched, has_leadership, has_communication
- Additional (4): publications, awards, projects, memberships
- Personality (4): exact_match, partial_match, introversion, thinking

Features are normalized (ratios 0-1, counts capped, binary 0/1).
StandardScaler applies further normalization during training.
"""
...
class FeatureExtractor:
    ...
    def _extract_skills_features(self, cv_data: dict, sector: str) -> list[float]:
        ...
        has_license = (
            1.0 if driving and driving not in ["no", "none", "n/a", ""] else 0.0
        )
    ...
    def _extract_additional_features(self, cv_data: dict) -> list[float]:
        ...
        proj_count = (
            len([p for p in projects.split(",") if p.strip()]) if projects else 0
        )
...
def extract_features(
    cv_data: dict, sector: str = "", sector_data: dict = None
) -> list[float]:
    """
