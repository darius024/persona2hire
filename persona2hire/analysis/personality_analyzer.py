"""
Personality analysis using MBTI and Big Five frameworks.

Infers personality from CV text (descriptions, hobbies, skills) via word matching.

MBTI Analysis (16 types):
- I/E: Introversion vs Extroversion (energy direction)
- S/N: Sensing vs iNtuition (concrete vs abstract thinking)
- T/F: Thinking vs Feeling (logic vs values in decisions)
- J/P: Judging vs Perceiving (structured vs flexible lifestyle)

Process: Tokenize text → match against curated word lists → accumulate scores
for each dimension → highest score wins. Hobbies also map to indicators.

Big Five (OCEAN): Openness, Conscientiousness, Extroversion, Agreeableness,
Neuroticism. Scored -100 to +100 based on high/low indicator matches.

Used for personality bonus in job scoring: exact MBTI match = +5 pts,
partial match (first 2 letters) = +2.5 pts.

Limitations: Text-based inference is imprecise, not a valid psych assessment.
Limited signal from short descriptions. Western/English word associations only.
"""
